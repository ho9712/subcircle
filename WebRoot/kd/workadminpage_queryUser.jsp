			...
			<ul id="browserItemList" class="browserList">
				<c:forEach items="${users }" var="user">
					...
					<!-- 头像 -->
					   	...
					   	<c:choose>
			            	<c:when test="${!empty user.kkd108}">
			           		 <a href="<%=path%>/kd01FindOther.kdhtml?kkd101=${user.kkd101 }" class="avatar">
			                	<span class="avatarNeue avatarSize48 ll" style="background-image:url('${user.kkd108}');background-size:100% auto;"></span>
			                </a>
			            	</c:when>
			            	<c:otherwise>
			           		<a href="<%=path%>/kd01FindOther.kdhtml?kkd101=${user.kkd101 }" class="avatar">
				    ...
				    <div class="inner">
				    	<!-- 用户信息 -->
				    	<span class="userInfo">
					    	<strong><a href="<%=path%>/kd01FindOther.kdhtml?kkd101=${user.kkd101 }" class="l">${user.kkd105 }</a></strong>
				    		<a href="<%=path%>/kd/message_send.jsp?kkd102=${user.kkd102 }"  class="tip_i icons_cmt">发消息</a>  
