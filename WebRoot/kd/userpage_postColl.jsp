...
<script type="text/javascript">
	function onClickSort(vorder)
	{
		...
		vform.action="<%=path%>/ka04PostColl.kdhtml?page=${param.page}&order="+vorder;
		vform.submit();
	}
	function delPost(kka401)
	{
		var msg="确定取消收藏该帖子?";
		if(confirm(msg))
		{
			window.location.href="<%= path %>/ka04DelPostColl.kdhtml?page=${param.page}&order=${param.order}&kka401="+kka401;		
			alert("取消成功！");
		}
	}
</script>
	...
	<div class="navSubTabsWrapper">
		<ul class="navSubTabs">
			<li><a href="<%=path%>/ka01PostRecord.kdhtml"><span>发帖记录</span></a></li>
  	 		...
  	 		<li><a href="<%=path%>/ka04PostColl.kdhtml" class="focus"><span>贴子收藏</span></a></li>
		</ul>
	</div>
	<!-- 子导航栏 -->
...
<div class="columns clearit" xmlns:v="http://rdf.data-vocabulary.org/#">
	<!-- 主页主体A栏 -->
    	...
    	<div id="browserTools" class="clearit">
			...
			<!-- 查询所有发帖记录 -->
			<c:forEach items="${postColls }" var="postColl">
			<div id="entry_list">
			<div class="item clearit">
			<a href="#" onclick="delPost('${postColl.kka401}')" class="btnGraySmall rr"><span>取消收藏</span></a>
			<h2 class="title">贴子: 
			<a href="<%= path %>/ka01PostContent.kahtml?kka101=${postColl.kka101}" class="l"> ${postColl.kka102 }</a>
			</h2>
			<div class="time"><small class="time">收藏时间: ${postColl.kka402 }</small> </div>
			<div class="content"><p class="line_limit">发帖人: ${postColl.kkd105 }</p>
			</div>
			<div class="tools clearit">
				<div class="tags">所属板块: 
					<a href="ka01AnimeForum.kahtml?id=${postColl.kka103 }" class="l">${postColl.cnkka103 }</a> 
				</div>
			</div>
			</div>
			</div>
								...
								<c:if test="${totalCount.count/6 >1}">
									<c:forEach begin="2" end="10" var="p">
										<c:choose>
											...
											<c:otherwise>
												<a href="<%=path%>/ka04PostColl.kdhtml?page=${p }&order=${param.order}" class="p">${p }</a>
											</c:otherwise>
										</c:choose>
									</c:forEach>
								</c:if>
								<a href="<%=path%>/ka04PostColl.kdhtml?page=2&order=${param.order}" class="p">&rsaquo;&rsaquo;</a>
							</c:when>
							<c:otherwise>
								<!-- 左移 -->
								<c:if test="${param.page >1}">
									<a href="<%=path%>/ka04PostColl.kdhtml?page=${param.page-1}&order=${param.order}" class="p">&lsaquo;&lsaquo;</a>
								</c:if>
								<!-- 左移 -->
								...
								<c:if test="${param.page < totalCount.count/6}">
									<a href="<%=path%>/ka04PostColl.kdhtml?page=${param.page+1}&order=${param.order}" class="p">&rsaquo;&rsaquo;</a>
								</c:if>
								<!-- 右移 -->
							</c:otherwise>
						</c:choose>
		...
		<div id="columnSubjectBrowserB" class="column">
			<div id="columnSubjectInBrowserB" class="clearit">
				<div class="SidePanel png_bg">
					<h2>/ 根据条件检索 </h2>
					<form id="myform" name="set" method="post" action="<%=path%>/ka04PostColl.kdhtml">
						<span class="text">
...
<c:choose>
	<c:when test="${empty hint }">
		<strong>『贴子收藏』</strong><br />
		这里记录了你在讨论版中收藏了的贴子。 <br />
	</c:when>
	<c:otherwise>
