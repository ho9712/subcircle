...
<script type="text/javascript">
	function onClickSort(vorder)
	{
		...
		vform.action="<%=path%>/kc06OtherGameColl.kdhtml?kkd101=${param.kkd101}&page=${param.page}&order="+vorder;
		vform.submit();
	}
</script>
<title>动画收藏</title>
								...
								<c:if test="${totalCount.count/6 >1}">
									<c:forEach begin="2" end="10" var="p">
										<c:choose>
											...
											<c:otherwise>
												<a href="<%=path%>/kc06OtherGameColl.kdhtml?kkd101=${param.kkd101}&page=${p }&order=${param.order}" class="p">${p }</a>
											</c:otherwise>
										</c:choose>
									</c:forEach>
								</c:if>
								<a href="<%=path%>/kc06OtherGameColl.kdhtml?kkd101=${param.kkd101}&page=2&order=${param.order}" class="p">&rsaquo;&rsaquo;</a>
							</c:when>
							<c:otherwise>
								<!-- 左移 -->
								<c:if test="${param.page >1}">
									<a href="<%=path%>/kc06OtherGameColl.kdhtml?kkd101=${param.kkd101}&page=${param.page-1}&order=${param.order}" class="p">&lsaquo;&lsaquo;</a>
								</c:if>
								<!-- 左移 -->
								...
								<c:if test="${param.page < totalCount.count/6}">
									<a href="<%=path%>/kc06OtherGameColl.kdhtml?kkd101=${param.kkd101}&page=${param.page+1}&order=${param.order}" class="p">&rsaquo;&rsaquo;</a>
								</c:if>
								<!-- 右移 -->
							</c:otherwise>
						</c:choose>
