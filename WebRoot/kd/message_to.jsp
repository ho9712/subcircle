	...
	<div class="navSubTabsWrapper">
		<ul class="navSubTabs">
			...
			<li><a href="<%=path%>/kd/message_send.jsp"><span>发送消息</span></a></li>
  	 		<li><a href="<%= path %>/kd02QuerySend.kdhtml?username=${user.kkd102}" class="focus"><span>已发送</span></a></li>
		</ul>
	</div>
	<!-- 子导航栏 -->
		...
		<div id="columnSubjectBrowserB" class="column">
			<div id="columnSubjectInBrowserB" class="clearit">
				<div class="SidePanel png_bg">
					<h2>/ 根据条件检索 </h2>
					<form id="myform" name="set" method="post" action="<%= path %>/kd02QuerySend.kdhtml">
						<input type="hidden" name="username" value="${user.kkd102 }">
						<span class="text">
							<table align="center" width="98%" cellspacing="0" cellpadding="5" class="settings">
								<tr>
									<td valign="top" width="15%">收信人</td>
									<td valign="top">
										<e:text name="receiver" style="border:1px
...
<c:choose>
	<c:when test="${empty hint }">
		<strong>『已发送』</strong><br />
		在这里，你可以查看你发送给其他用户的消息。<br />
	</c:when>
	<c:otherwise>
