...
<script type="text/javascript">
	function onClickSort(vorder)
	{
		...
		vform.action="<%=path%>/kc06OtherBookColl.kdhtml?kkd101=${param.kkd101}&page=${param.page}&order="+vorder;
		vform.submit();
	}
</script>
<title>动画收藏</title>
	...
	<div class="navSubTabsWrapper">
		<ul class="navSubTabs">
			<li><a href="<%=path%>/kc06OtherAnimeColl.kdhtml?kkd101=${param.kkd101}"><span>动画</span></a></li>
  	 		<li><a href="<%=path%>/kc06OtherBookColl.kdhtml?kkd101=${param.kkd101}" class="focus"><span>书籍</span></a></li>
  	 		<li><a href="<%=path%>/kc06OtherGameColl.kdhtml?kkd101=${param.kkd101}" ><span>游戏</span></a></li>
		</ul>
	</div>
	<!-- 子导航栏 -->
...
<div class="columns clearit" xmlns:v="http://rdf.data-vocabulary.org/#">
	<!-- 主页主体A栏 -->
		...
		<c:when test="${empty bookColls}">
			<div class="section">
	            <br><h2 class="title">Ta还没有收藏过书籍...</h2>
	        	<div>
								...
								<c:if test="${totalCount.count/6 >1}">
									<c:forEach begin="2" end="10" var="p">
										<c:choose>
											...
											<c:otherwise>
												<a href="<%=path%>/kc06OtherBookColl.kdhtml?kkd101=${param.kkd101}&page=${p }&order=${param.order}" class="p">${p }</a>
											</c:otherwise>
										</c:choose>
									</c:forEach>
								</c:if>
								<a href="<%=path%>/kc06OtherBookColl.kdhtml?kkd101=${param.kkd101}&page=2&order=${param.order}" class="p">&rsaquo;&rsaquo;</a>
							</c:when>
							<c:otherwise>
								<!-- 左移 -->
								<c:if test="${param.page >1}">
									<a href="<%=path%>/kc06OtherBookColl.kdhtml?kkd101=${param.kkd101}&page=${param.page-1}&order=${param.order}" class="p">&lsaquo;&lsaquo;</a>
								</c:if>
								<!-- 左移 -->
								...
								<c:if test="${param.page < totalCount.count/6}">
									<a href="<%=path%>/kc06OtherBookColl.kdhtml?kkd101=${param.kkd101}&page=${param.page+1}&order=${param.order}" class="p">&rsaquo;&rsaquo;</a>
								</c:if>
								<!-- 右移 -->
							</c:otherwise>
						</c:choose>
		...
		<div id="columnSubjectBrowserB" class="column">
			<div id="columnSubjectInBrowserB" class="clearit">
				<div class="SidePanel png_bg">
					<h2>/ 根据条件检索 </h2>
					<form id="myform" name="set" method="post" action="<%=path%>/kc06OtherBookColl.kdhtml?kkd101=${param.kkd101}">
						<span class="text">
...
<c:choose>
	<c:when test="${empty hint }">
		<strong>『Ta的书籍收藏』</strong><br />
		这里显示了Ta所收藏的所有书籍作品。 <br />
	</c:when>
	<c:otherwise>
