...
<script type="text/javascript">
	function onClickSort(vorder)
	{
		...
		vform.action="<%=path%>/kc06AnimeColl.kdhtml?page=${param.page}&order="+vorder;
		vform.submit();
	}
	...
	function delColl(kkc601,kkc701)
	{
		...
		if(confirm(msg))
		{
			window.location.href="<%=path%>/kc06DelAnimeColl.kdhtml?page=${param.page}&kkc601="+kkc601+"&kkc701="+kkc701;		
			alert("取消收藏成功！");
		}
	}
</script>
...
<div id="headerProfile" class="clearit">
	...
	<div class="navSubTabsWrapper">
		<ul class="navSubTabs">
			<li><a href="<%=path%>/kc06AnimeColl.kdhtml" class="focus"><span>动画</span></a></li>
  	 		<li><a href="<%=path%>/kc06BookColl.kdhtml" ><span>书籍</span></a></li>
...
<div class="columns clearit" xmlns:v="http://rdf.data-vocabulary.org/#">
	<!-- 主页主体A栏 -->
		...
		<c:choose>
		<c:when test="${empty animeColls}">
			<div class="section">
				<a href="<%=path%>/kc02ShowRank.kchtml" class="rr l">前往收藏</a>
	            <br><h2 class="title">你还没有收藏过动画...</h2>
	        	<div>
    	...
    	<div id="browserTools" class="clearit">
			...
			<ul id="browserItemList" class="browserFull">
				<c:forEach items="${animeColls }" var="animeColl">
				<li id="item_9912" class="item odd clearit">
			    <a href="Kc02ShowDetail.kchtml?kkc101=${animeColl.kkc101}" class="subjectCover cover ll">       
		            <span class="image">
	                       <img src="${animeColl.kkc204 }" alt="${animeColl.kkc204 }" class="cover" />
			        </span>
			    ...
			    <div class="inner">
				    ...
				    <a href="#TB_inline?height=350&amp;width=500&amp;inlineId=panel"
	                    onclick="modifyComment('${animeColl.kkc701}','${animeColl.kkc702}','${animeColl.kkc703}')" title="修改评价"  class="thickbox l">修改</a> | 
				    <a href="#;" onclick="delColl('${animeColl.kkc601}','${animeColl.kkc701}')" class="l">删除</a>
				    </p>
				    </div>
	                <h3>
	                    <a href="Kc02ShowDetail.kchtml?kkc101=${animeColl.kkc101}"  class="l">${animeColl.kkc202 }</a> 
	                </h3>
			        <p class="info tip">${animeColl.kkc205 }</p>
			        <p class="collectInfo">
					<span class="sstars${animeColl.kkc702 } starsinfo"></span><span class="tip_j"> ${animeColl.kkc602 }</span> 
					</p>
					<div id="comment_box">
	            	<div class="item">
	            		<div class="text_main_even" style="float:none;">
	            		<div class="text">${animeColl.kkc703 }</div>
		...
		<div id="panel" style="display:none;"><div class="collectBox clearit">
		    <form id="collectBoxForm" name="collectBoxForm" method="post" action="kc06ModifyAnimeComment.kdhtml?page=${param.page}&order=${param.order}">
								...
								<c:if test="${totalCount.count/6 >1}">
									<c:forEach begin="2" end="10" var="p">
										<c:choose>
											...
											<c:otherwise>
												<a href="<%=path%>/kc06AnimeColl.kdhtml?page=${p }&order=${param.order}" class="p">${p }</a>
											</c:otherwise>
										</c:choose>
									</c:forEach>
								</c:if>
								<a href="<%=path%>/kc06AnimeColl.kdhtml?page=2&order=${param.order}" class="p">&rsaquo;&rsaquo;</a>
							</c:when>
							<c:otherwise>
								<!-- 左移 -->
								...
								<c:if test="${param.page < totalCount.count/6}">
									<a href="<%=path%>/kc06AnimeColl.kdhtml?page=${param.page+1}&order=${param.order}" class="p">&rsaquo;&rsaquo;</a>
								</c:if>
								<!-- 右移 -->
							</c:otherwise>
						</c:choose>
		...
		<div id="columnSubjectBrowserB" class="column">
			<div id="columnSubjectInBrowserB" class="clearit">
				<div class="SidePanel png_bg">
					<h2>/ 根据条件检索 </h2>
					<form id="myform" name="set" method="post" action="<%=path%>/kc06AnimeColl.kdhtml">
						<span class="text">
