...
<script type="text/javascript">
	function onClickSort(vorder)
	{
		...
		vform.action="<%=path%>/ka01OtherPostRecord.kdhtml?kkd101=${param.kkd101}&page=${param.page}&order="+vorder;
		vform.submit();
	}
</script>
<style type="text/css">
...
<div id="headerProfile" class="clearit">
	...
	<div class="navSubTabsWrapper">
		<ul class="navSubTabs">
			<li><a href="<%=path%>/ka01OtherPostRecord.kdhtml?kkd101=${param.kkd101}" class="focus"><span>发帖记录</span></a></li>
  	 		<li><a href="<%=path%>/ka02OtherReplyRecord.kdhtml?kkd101=${param.kkd101}" ><span>回帖记录</span></a></li>
		</ul>
	</div>
	<!-- 子导航栏 -->
...
<div class="columns clearit" xmlns:v="http://rdf.data-vocabulary.org/#">
	<!-- 主页主体A栏 -->
		...
		<c:when test="${empty posts}">
			<div class="section">
	            <br><h2 class="title">Ta还没有发表过贴子...</h2>
	        	<div>
								...
								<c:if test="${totalCount.count/6 >1}">
									<c:forEach begin="2" end="10" var="p">
										<c:choose>
											...
											<c:otherwise>
												<a href="<%=path%>/ka01OtherPostRecord.kdhtml?kkd101=${param.kkd101}&page=${p }&order=${param.order}" class="p">${p }</a>
											</c:otherwise>
										</c:choose>
									</c:forEach>
								</c:if>
								<a href="<%=path%>/ka01OtherPostRecord.kdhtml?kkd101=${param.kkd101}&page=2&order=${param.order}" class="p">&rsaquo;&rsaquo;</a>
							</c:when>
							<c:otherwise>
								<!-- 左移 -->
								<c:if test="${param.page >1}">
									<a href="<%=path%>/ka01OtherPostRecord.kdhtml?kkd101=${param.kkd101}&page=${param.page-1}&order=${param.order}" class="p">&lsaquo;&lsaquo;</a>
								</c:if>
								<!-- 左移 -->
								...
								<c:if test="${param.page < totalCount.count/6}">
									<a href="<%=path%>/ka01OtherPostRecord.kdhtml?kkd101=${param.kkd101}&page=${param.page+1}&order=${param.order}" class="p">&rsaquo;&rsaquo;</a>
								</c:if>
								<!-- 右移 -->
							</c:otherwise>
						</c:choose>
		...
		<div id="columnSubjectBrowserB" class="column">
			<div id="columnSubjectInBrowserB" class="clearit">
				<div class="SidePanel png_bg">
					<h2>/ 根据条件检索 </h2>
					<form id="myform" name="set" method="post" action="<%=path%>/ka01OtherPostRecord.kdhtml?kkd101=${param.kkd101}">
						<span class="text">
...
<c:choose>
	<c:when test="${empty hint }">
		<strong>『Ta的发帖记录』</strong><br />
		这里记录了Ta曾经在讨论版发表过的贴子。 <br />
	</c:when>
	<c:otherwise>
